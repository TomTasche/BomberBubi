(function initGame() {
    var socket, playerId, buildArena, sendMovement, lastMovement, lockMovement = false,
        calculateGridDistance, calculatePlayerDistance, calculateDistance, lockBomb = false,
        onKeyUp, toggleMovementLock, toggleBombLock, changeType, canvas = document.getElementById('canvas'),
    ...
    buildArena = function buildArena(size) {
        var length = calculateDistance(size) + 4,
            i = 0,
            distance = 0;
    ...
    makeMove = function makeMove() {
        ...
        window.setTimeout(toggleMovementLock, 100);
    }
    ...
    window.setInterval(makeMove, 100);
})();
